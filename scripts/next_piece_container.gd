...
const DECK_SELECTOR_SCENE: PackedScene = preload("res://scenes/modal/deck_selector/deck_selector_screen.tscn")
const HOVER_MODULATE: Color = Color(1.2, 1.2, 1.2, 1)
const NEXT_CONTAINER_SIZE: int = 48

var scale_tween: Tween
var deck_selector_instance: DeckSelectorScreen = null
...
func center_piece(piece_type: PieceRenderer.ShapeType) -> void:
	...
	var piece_pixel_width: float = bounds.size.x * PieceRenderer.PIECE_SIZE
	var piece_pixel_height: float = bounds.size.y * PieceRenderer.PIECE_SIZE

	var center_x: float = (next_container.size.x - piece_pixel_width) / 2.0
	var center_y: float = (next_container.size.y - piece_pixel_height) / 2.0

	var offset_x: float = center_x - (bounds.position.x * PieceRenderer.PIECE_SIZE)
	var offset_y: float = center_y - (bounds.position.y * PieceRenderer.PIECE_SIZE)
...
func _pressed() -> void:
	if is_instance_valid(deck_selector_instance):
		return
	...
	deck_selector_instance = DECK_SELECTOR_SCENE.instantiate() as DeckSelectorScreen
	var hud: Node = GameManager.get_current_scene().get_node_or_null("HUD")
	if hud:
		hud.add_child(deck_selector_instance)
