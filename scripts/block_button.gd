...
func _ready() -> void :
	piece_renderer.set_piece(PieceRenderer.ShapeType.T, 0, false, type)
	center_piece(PieceRenderer.ShapeType.T)
...
func get_balanced_shape_selection(shapes_count: int) -> Array[PieceRenderer.ShapeType]:
	var selected_shapes: Array[PieceRenderer.ShapeType] = []
	var all_shapes: Array = PieceRenderer.ShapeType.values()
